...
test.describe('Settings Blocks', () => {
  test.describe('Profile Block', () => {
    ...
    test('should have avatar or profile image section', async ({ page }) => {
      const avatar = page.locator(
        'img[alt*="avatar" i], img[alt*="profile" i], [class*="avatar"], [class*="profile-image"]',
      );
    ...
    test('should have bio or description field', async ({ page }) => {
      const bioField = page.locator(
        'textarea, input[placeholder*="bio" i], input[placeholder*="about" i]',
      );
  ...
  test.describe('Notifications Block', () => {
    ...
    test('should render notification settings', async ({ page }) => {
      const container = page
        .locator('[class*="notification"], [class*="settings"], main')
        .first();
    ...
    test('should have notification categories', async ({ page }) => {
      const categories = page.locator(
        'h3, h4, [class*="category"], [class*="section-title"]',
      );
  ...
  test.describe('Account Block', () => {
    ...
    test('should render account settings', async ({ page }) => {
      const container = page
        .locator('[class*="account"], [class*="settings"], main, form')
        .first();
      await expect(container).toBeVisible();
    });
    ...
    test('should have password change section', async ({ page }) => {
      const passwordSection = page.locator(
        'input[type="password"], [class*="password"], h3:has-text("Password"), h4:has-text("Password")',
      );
    ...
    test('should have delete account option', async ({ page }) => {
      const deleteOption = page.locator(
        'button:has-text("Delete"), button:has-text("Remove"), [class*="danger"], [class*="delete"]',
      );
    ...
    test('should have two-factor authentication section', async ({ page }) => {
      const twoFactorSection = page.locator(
        '[class*="2fa"], [class*="two-factor"], text=Two-factor, text=2FA',
      );
    ...
    test('should have session management', async ({ page }) => {
      const sessionSection = page.locator(
        '[class*="session"], [class*="device"], text=Session, text=Device',
      );
