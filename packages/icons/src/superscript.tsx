import { JSX } from 'solid-js';
import { IconProps, IconWrapper } from './components/IconWrapper';

export const SuperscriptIcon = (props: IconProps): JSX.Element => (
  <IconWrapper {...props}>
    <path d="m4 19 8-8" stroke-linecap="round" stroke-linejoin="round" />
    <path d="m12 19-8-8" stroke-linecap="round" stroke-linejoin="round" />
    <path d="M20 12h-4c0-1.5.442-2 1.5-2.5S20 8.334 20 7.002c0-.472-.17-.93-.484-1.29a2.105 2.105 0 0 0-2.617-.436c-.42.239-.738.614-.899 1.06" stroke-linecap="round" stroke-linejoin="round" />
  </IconWrapper>
);
