 ...
 */
const getFileCategory = (file: File): 'image' | 'video' | 'audio' | 'pdf' | 'archive' | 'code' | 'document' => {
...
export const UploadFile = (props: UploadFileProps): JSX.Element => {
  ...
  const updateTrackedFile = (id: string, updates: Partial<TrackedFile>) => {
    setTrackedFiles((prev) => prev.map((tf) => (tf.id === id ? { ...tf, ...updates } : tf)));
  };
  ...
  const processFiles = (files: FileList) => {
    ...
    if (props.maxLength && totalFiles > props.maxLength) {
      toast[props.toastKey ?? 'error']?.(props.maxLengthError ?? `Maximum ${props.maxLength} files allowed`);
      return;
    }
    ...
    fileArray.forEach((file) => {
      if (!isFileTypeAllowed(file)) {
        toast[props.toastKey ?? 'error']?.(
          props.fileTypeError ? props.fileTypeError(file) : `File type not allowed: ${file.name} (${file.type})`,
        );
        return;
      }
      ...
      if (props.maxSize && file.size > Number(props.maxSize)) {
        toast[props.toastKey ?? 'error']?.(
          props.maxSizeError ? props.maxSizeError(file) : `File too large: ${file.name} (${formatFileSize(file.size)})`,
        );
        return;
      }
    ...
    const updatedFiles = props.multiple ? [...currentFiles, ...newTrackedFiles] : newTrackedFiles.slice(0, 1);
  ...
  const FileIcon = (iconProps: { file: File; class?: string }) => {
    ...
    return (
      <Switch fallback={<File04Icon strokeWidth={1.5} class={iconProps.class ?? 'size-8'} />}>
        <Match when={category() === 'image'}>
          <Image01Icon strokeWidth={1.5} class={`${iconProps.class ?? 'size-8'} text-purple-500`} />
        </Match>
        <Match when={category() === 'video'}>
          <VideoRecorderIcon strokeWidth={1.5} class={`${iconProps.class ?? 'size-8'} text-pink-500`} />
        </Match>
        <Match when={category() === 'audio'}>
          <MusicNote01Icon strokeWidth={1.5} class={`${iconProps.class ?? 'size-8'} text-orange-500`} />
        </Match>
        <Match when={category() === 'pdf'}>
          <File04Icon strokeWidth={1.5} class={`${iconProps.class ?? 'size-8'} text-red-500`} />
        </Match>
        <Match when={category() === 'archive'}>
          <ArchiveIcon strokeWidth={1.5} class={`${iconProps.class ?? 'size-8'} text-yellow-600`} />
        </Match>
        <Match when={category() === 'code'}>
          <FileCode01Icon strokeWidth={1.5} class={`${iconProps.class ?? 'size-8'} text-green-500`} />
        </Match>
      </Switch>
    );
  };
    ...
    <div class="w-full">
      ...
      >
        <div class="px-8 py-8 text-center">
          ...
          {/* Text */}
          <div class="mb-2">
            <span class="text-gray-600 dark:text-gray-300">{props.dragDropText ?? 'Drag and drop your files here'}</span>
          </div>
          ...
          <Show when={props.helperText}>
            <p class="mt-4 text-xs text-gray-500 dark:text-gray-400">{props.helperText}</p>
          </Show>
        </div>
        ...
        <Show when={dragActive()}>
          <div
            ...
            onDrop={handleDrop}
          >
            <div class="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-lg">Drop files here</div>
          </div>
        </Show>
      </div>
            ...
            {(trackedFile) => (
              ...
              >
                <div class="flex items-center gap-4 p-4">
                  ...
                  <div class="shrink-0">
                    <Show
                      when={trackedFile.previewUrl && getFileCategory(trackedFile.file) === 'image'}
                      fallback={
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="flex items-start justify-between gap-2">
                      <div class="min-w-0 flex-1">
                        <p class="truncate text-sm font-medium text-gray-900 dark:text-white">{trackedFile.file.name}</p>
                        <div class="mt-0.5 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                          <span>{formatFileSize(trackedFile.file.size)}</span>
                          <Show when={trackedFile.state === 'uploading' && trackedFile.speed}>
                            <span class="text-gray-300 dark:text-gray-600">•</span>
                            <span class="text-blue-600 dark:text-blue-400">{formatSpeed(trackedFile.speed!)}</span>
                          </Show>
                          <Show when={trackedFile.state === 'uploading' && trackedFile.timeRemaining}>
                            <span class="text-gray-300 dark:text-gray-600">•</span>
                            <span>{formatTimeRemaining(trackedFile.timeRemaining!)}</span>
                          </Show>
                        </div>
                        <Show when={trackedFile.state === 'error' && trackedFile.errorMessage}>
                          <p class="mt-1 text-xs text-red-600 dark:text-red-400">{trackedFile.errorMessage}</p>
                        </Show>
                      </div>
                ...
                <Show when={trackedFile.state === 'success'}>
                  <div class="absolute inset-0 pointer-events-none">
                    <div class="absolute inset-0 animate-pulse bg-gradient-to-r from-green-500/5 via-green-500/10 to-green-500/5" style={{ "animation-duration": "1s", "animation-iteration-count": "1" }} />
                  </div>
                </Show>
              </div>
            )}
