...
| Minor | 1.4.11 | Search clear button `text-gray-400` -- ~2.7:1 contrast, below 3:1 for UI components | 426 |
...
| Minor | 1.4.11 | Interactive text using `text-gray-500` (~4.6:1) -- borderline | 579, 679, 698 |
...
| Severity | WCAG | Description | Lines |
|----------|------|-------------|-------|
| Minor | 1.4.11 | Default `bg-gray-100` on white has ~1.06:1 contrast -- placeholder shape may be invisible to low-vision users | 77 |
| Minor | 4.1.1 | Dynamic Tailwind class `bg-gray-${gray()}` won't be detected by JIT purge -- may be missing in production | 77 |
